...
A skill `/sdd:spawn-sdd-worker` exists in the SDD plugin. When invoked with a domain argument, it uses the Agent tool to spawn a persistent agent named `sdd-worker`. The agent's prompt instructs it to run the full execution pipeline for the given domain by invoking `sdd:close-domain {domain}` as its first action — the loop itself (audit → decompose → close → guardian audit) lives inside close-domain (SPEC-wf-038), not in the prompt. The worker is not a one-shot subagent — it is named so it can be sent additional domains via `SendMessage` (running close-domain for each) and reused across pipeline runs in the same session.
...
- The worker prompt instructs invoking `sdd:close-domain {domain}` as the worker's first action, with no other procedure in the prompt
- The worker can receive additional domains via `SendMessage` without re-spawning, running `sdd:close-domain` for each
