...
User-authored coding standards (SPEC-wf-028) are enforced through three complementary layers, none of which is `/sdd:spec-audit`: (1) **proactive** — `sdd:session-start` surfaces the `.sdd/standards/` rubric into the agent's working context at session open so it follows the standards as it writes; (2) **active blocking** — linters/formatters plus a hook or CI gate reject code that violates the mechanically-checkable rules; (3) **review-time** — the issues reviewer team (SPEC-wf-025) audits code against the `.sdd/standards/` rubric for the judgement-based rules linters cannot express, and the resulting issues flow into refactor work items. Mechanically-checkable rules are routed to the lint/blocking layer; judgement-based rules (anti-patterns, architectural rules) are routed to the reviewer.

## Acceptance criteria

- `sdd:session-start` surfaces the `.sdd/standards/` rubric into the agent's working context so the standards are followed during authoring
...
**Tests:**
- `plugin/skills/session-start/SKILL.md` references ".sdd/standards/" — "proactive layer: session-start surfaces the standards rubric into the agent's context"
