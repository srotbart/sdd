...
**Tests:**
hub/server/spec-arch.test.ts > SPEC-arch-019: per-spec test mapping file > SPEC-arch-019: readTestMapping parses runner, report and items from SPEC-{abbrev}.tests.json — "the test mapping file is parsed into runner, report and item mappings"
hub/server/spec-arch.test.ts > SPEC-arch-019: per-spec test mapping file > SPEC-arch-019: returns null when no companion mapping file exists — "a missing mapping file yields no mapping"
