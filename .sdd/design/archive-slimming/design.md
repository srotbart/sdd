...
| Long-term retention ("what if?") | Terminal state committed **before** archiving → git history retains full content of merged work; recovery is a manual `git log`/`git show` when ever needed |
...
- Archiving skills keep the current behavior — flip terminal state, `mv` the
  file into the type's `archive/` dir — with one new rule: **commit the
  terminal state before the `mv`**. This keeps git history a complete record
  of every artifact's final content; if an archived artifact is ever needed
  again, manual recovery is `git log --diff-filter=A -- <path>` +
  `git show <sha>:<path>`. (Caveats: holds for merged work under the current
  merge-commit strategy; squash merges or rebasing artifact commits away
  would lose created-and-closed-in-one-PR artifacts.)
...
| Surface | Change |
|---|---|
| `work-item-close`, `target-engage`, `review-engage` (archiving flows) | Commit terminal state before the `mv`; stop staging archive paths |
