...
minting the ID with the artifact CLI (`node plugin/cli/sdd.js mint
improvement {abbrev}`, resolving the script from the repo or the installed plugin
cache at `$HOME/.claude/plugins/cache/sdd/sdd/*/cli/sdd.js`; fallback:
`{7hex}` is 7 random lowercase hex characters, e.g. `openssl rand -hex 4 |
cut -c1-7`) — no sequence scan, collision-free by construction. Existing
sequential `IMP-{abbrev}-{seq}` IDs remain valid and are never renamed.
